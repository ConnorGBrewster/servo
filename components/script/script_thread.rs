...
impl InProgressLoad {
    ...
    fn new(id: PipelineId,
           ...
           window_size: Option<WindowSizeData>,
           url: ServoUrl,
           origin: ServoOrigin) -> InProgressLoad {
        InProgressLoad {
            pipeline_id: id,
            frame_id: frame_id,
...
impl ScriptThreadFactory for ScriptThread {
    ...
    fn create(state: InitialScriptState,
        ...
        thread::Builder::new().name(format!("ScriptThread {:?}", state.id)).spawn(move || {
            ...
            let origin = ServoOrigin::new(&load_data.url);
...
impl ScriptThread {
    ...
    pub fn process_attach_layout(new_layout_info: NewLayoutInfo, origin: ServoOrigin) {
        SCRIPT_THREAD_ROOT.with(|root| {
            if let Some(script_thread) = root.get() {
                let script_thread = unsafe { &*script_thread };
            ...
            match event {
                ...
                FromConstellation(ConstellationControlMsg::AttachLayout(
                    ...
                    self.profile_event(ScriptThreadEventCategory::AttachLayout, || {
                        let origin = ServoOrigin::new(&new_layout_info.load_data.url);
    ...
    fn handle_new_layout(&self, new_layout_info: NewLayoutInfo, origin: ServoOrigin) {
        let NewLayoutInfo {
            parent_info,
            new_pipeline_id,
            frame_id,
